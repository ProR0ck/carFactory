...
LEFT JOIN `engineering_staff` e ON w.`workshop_manager`=e.`id_worker`");
				...
				{
					echo "<div class='workshop'> <a href='workshops/workshops.php?search={$workshop[1]}&submit='>{$workshop[1]}</a><br>Начальник цеха:<a href='engineering_staff/engineering_info.php?id_info={$workshop[2]}'>{$workshop[3]} {$workshop[4]} {$workshop[5]}</a><br>";
					...
					{
						echo "<div class='plot'><b><a href='plots/plots.php?search={$plot[1]}&submit='>{$plot[1]}</a></b><br>Вид работ: {$plot[6]} <br>Начальник участка:<br><b><a href='engineering_staff/engineering_info.php?id_info={$plot[2]}'> {$plot[3]} {$plot[4]} {$plot[5]}</a></b><br>";
